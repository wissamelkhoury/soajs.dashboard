...
var BL = {
	...
	"delete": function (config, req, res, cbMain) {
		...
		function checkContainerDeployments(envRecord, cb) {
			...
			deployer.listServices(options, function (error, services) {
				utils.checkErrorReturn(req, cbMain, {config: config, error: error}, function () { //utils.checkErrorReturn() is used to handle driver errors
	...
	"addDb": function (config, req, res, cbMain) {
		...
		checkReturnError(req, cbMain, {
			config: config,
			error: req.soajs.inputmaskData.usedForAnalytics && req.soajs.inputmaskData.env.toUpperCase() !== 'DASHBOARD',
			code: 550
		}, function () {
			BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
				checkReturnError(req, cbMain, {config: config, error: err || !envRecord, code: 402}, function () {
						...
						if (req.soajs.inputmaskData.name === 'session') {
							...
							checkReturnError(req, cbMain, {
								...
								envRecord.dbs.databases[req.soajs.inputmaskData.name] = {
									...
									'usedForAnalytics': req.soajs.inputmaskData.usedForAnalytics || false
	...
	"listClusters": function (config, req, res, cbMain) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkReturnError(req, cbMain, {config: config, error: err || !envRecord, code: 402}, function () {
				return cbMain(null, envRecord.dbs.clusters);
			});
		});
	},
	
	"addCluster": function (config, req, res, cbMain) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkReturnError(req, cbMain, {config: config, error: err || !envRecord, code: 402}, function () {
				...
				checkReturnError(req, cbMain, {
					...
					opts.record = envRecord;
					BL.model.saveEntry(req.soajs, opts, function (err) {
						checkReturnError(req, cbMain, {config: config, error: err, code: 505}, function () {
							return cbMain(null, "environment cluster added successful");
						});
					});
				});
			});
	...
	"updateCluster": function (config, req, res, cbMain) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (err, envRecord) {
			checkReturnError(req, cbMain, {config: config, error: err || !envRecord, code: 402}, function () {
				...
				checkReturnError(req, cbMain, {
					...
					opts.record = envRecord;
					BL.model.saveEntry(req.soajs, opts, function (err) {
						checkReturnError(req, cbMain, {config: config, error: err, code: 506}, function () {
							return cbMain(null, "environment cluster updated successful");
						});
					});
				});
			});
	...
	"deleteCluster": function (config, req, res, cbMain) {
		var opts = {};
		opts.collection = colName;
		opts.conditions = {'code': req.soajs.inputmaskData.env.toUpperCase()};
		BL.model.findEntry(req.soajs, opts, function (error, envRecord) {
			checkReturnError(req, cbMain, {config: config, error: error || !envRecord, code: 402}, function () {
				checkReturnError(req, cbMain, {
					...
					BL.model.saveEntry(req.soajs, opts, function (err) {
						checkReturnError(req, cbMain, {config: config, error: err, code: 402}, function () {
							return cbMain(null, "environment cluster removed successful");
						});
					});
				});
			});
