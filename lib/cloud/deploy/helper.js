...
var helpers = {
	getAnalyticsEsInfo: function (soajs, context, model, cb) {
		var env = process.env.SOAJS_ENV || 'dashboard'; //NOTE: analytics cluster config is restricted to dashboard environment
		utils.getEnvironment(soajs, model, env, function (error, envRecord) {
			if (error) {
				return cb(error);
			}

			if (!envRecord || !envRecord.dbs || !envRecord.dbs.databases) {
				return cb();
			}
