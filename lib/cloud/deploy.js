...
function computeCatalogEnvVars(context, soajs, config, cb) {
	...
	async.concat(catalogEnvs, function (oneEnvName, callback) {
		...
		// if env variable is of type userInput, get value from request body, if not found see use default value
			...
			if (soajs.inputmaskData.custom && soajs.inputmaskData.custom.env) {
				if (soajs.inputmaskData.custom.env[oneEnvName]) {
					value = soajs.inputmaskData.custom.env[oneEnvName];
				}
				else if (!soajs.inputmaskData.custom.env[oneEnvName] && oneEnv.required) {
...
function deployContainer(config, context, soajs, res, cbMain) {
	...
	function constructDeployerParams(serviceName) {
		...
		//only add the env code label for services that are per environment
		if (!soajs.inputmaskData.custom || (soajs.inputmaskData.custom && !soajs.inputmaskData.custom.allEnv)) {
			serviceParams.labels['soajs.env.code'] = context.envRecord.code.toLowerCase();
		}
		...
		function getAnalyticsEsInfo(cb) {
			...
			utils.getEnvironment(soajs, BL.model, env, function (error, envRecord) {
				...
				async.detect(envRecord.dbs.databases, function (oneDb, callback) {
					return callback(null, oneDb.usedForAnalytics);
				...
				switch (context.catalog.type) {
					...
					default:
						...
						if (soajs.inputmaskData.custom && soajs.inputmaskData.custom.name) {
							...
							serviceLabel = context.envRecord.code.toLowerCase() + "-" + context.catalog.type + "-" + serviceUid;
							if (soajs.inputmaskData.custom && soajs.inputmaskData.custom.allEnv) {
								serviceLabel = context.catalog.type + "-" + serviceUid;
							}
						}
						...
						else if(context.catalog.type.toLowerCase() === 'elk'){
							context.labels['soajs.content'] = "true";
							context.labels['soajs.service.group'] = "elk";
						}
